...
export function WorkshopRepositoryCommitNotification({
    ...
    return (
        <motion.aside
            ...
            className="pointer-events-auto absolute left-3 right-3 top-3 z-20 rounded-2xl border border-room-warning/50 bg-room-inset/95 p-3 shadow-2xl backdrop-blur sm:left-6 sm:right-32 sm:top-5 sm:max-w-2xl sm:p-4"
        >
            <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-room-warning">
            ...
            <WorkshopRepositoryCommitCard
                ...
                commit={commit}
                isNew
                className="mt-2 bg-room-warning/[0.04]"
            />
        </motion.aside>
    );
}
