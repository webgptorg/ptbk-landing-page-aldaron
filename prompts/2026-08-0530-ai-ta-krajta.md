...
[x] by OpenAI Codex `gpt-5.6-terra` thinking `max` (ChatGPT account) - Implementation ~$0.1158 5 minutes; Testing 9 minutes

[✨🧓] Make the `PromptbookCoderBadge` little bitl less obtrusive
...
[x] (2 attempts) by Qwen Code `qwen3.8-flash` thinking `max` - Implementation <$0.01 an hour; Testing an hour; Fixing <$0.01 an hour; Testing 18 minutes
...
- This is espetially problem with older episodes.
- You are working with page `/ai-ta-krajta`

---

[x] by Developer on OpenAI Codex `gpt-6-astra` thinking `max` (ChatGPT account) - Implementation ~$0.5111 20 minutes; Testing 6 minutes

[✨🧓] All people should have normalized pictures

- Each picture should be transparent PNG
- The backgrounds should be some neutral color gradients with some subtle variations
- When you hover over a picture, it should display a subtle highlight or animation to indicate interactivity.
- Go through all hosts one by one and ensure each has a normalized picture rendered perfectly on page
- You are working with page `/ai-ta-krajta`

![current situation](prompts/screenshots/2026-08-0530-ai-ta-krajta-16.png)
...
[x] by OpenAI Codex `gpt-5.6-terra` thinking `max` (ChatGPT account) - Implementation ~$0.3765 14 minutes; Testing 8 minutes
