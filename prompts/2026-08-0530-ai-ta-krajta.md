...
[x] by Claude Code `claude-opus-5` thinking `max` - Implementation 4.54 30 minutes; Testing 8 minutes
...
- When user does nothing, it should start sleeping
- It should track mouse movements but it some subtle way, without being intrusive.
- It should blink its eyes periodically but in a natural and non-distracting way.
...
- Hovering should not chamge the position or size of the badge.
- Add it also in footer with text "Done by Promptbook coder" and link to https://coder.ptbk.io/
- Encapsulate the badge functionality in a self-contained react component to be reusable on other pages as well.
